...
app.MapGet("/enqueue", (HttpContext context) =>
{
    var messageBody = new TriggerWorkflowMessage
    {
        ...
        TriggerData = "Trigger remote workflow from Client B"
    };

    var jobId = BackgroundJob.Enqueue<IRunRemoteWorkflow>(job => job.Run(QueueNames.ServerB, 22, messageBody, "MyKnownClientAV1Processor"));
